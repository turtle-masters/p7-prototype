...
public class ReactionScript : MonoBehaviour
{
    ...
    void React() {
        foreach(GameObject reactionObject in reactionSpawnObjects)
        {
            ...
            Instantiate(reactionObject,transform.position+randomOffset,Quaternion.identity).transform.SetParent(MinigameManagerScript.instance.GetCurrentLevelObject().transform);
        }
